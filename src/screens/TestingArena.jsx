...
// import { AiOutlineDelete, AiOutlineSync, AiOutlineEdit, AiFillQuestionCircle } from "react-icons/ai";
// import Toggle from "../components/Toggle";
// import Button from "../components/Button";
// const TestingArena = () => {
//   const HEAD_LABELS = ["REF.", "DATE", "CLIENT", "TYPE D'ÉVÉNEMENT", "LIEU", "STATUS"].map((label) => {
//     return {
//       contentType: "head",
//       contentValue: label,
//     }
//   });
//   const formatRowForTable = ([ref, date, name, type, location, status]) => {
//     return [
//       {
//         contentType: "link",
//         contentValue: {
//           href: "/selectedAppointment",
//           text: ref,
//         },
//       },
//       {
...
//     ]
//   }
//   const BODY_ROWS = new Array(6).fill(["05527", "Oct 14, 2023", "Antoinette Porter", "Shooting", "72 Cody Redge Road", "waiting"]).map(formatRowForTable);
//   const onClick = () => {};
//   return <div className="w-full h-full p-4 bg-primary text-strong-contrast flex flex-col gap-4">
//     <div className="w-full flex justify-between items-center">
//       <Toggle id="darkMode"/>
//       <Button onClick={onClick} textContent="Click me!"/>
//       <Button onClick={onClick} textContent="Click Icon'ed!" Icon={AiFillQuestionCircle}/>
//     </div>
//     <Table rows={BODY_ROWS} head={HEAD_LABELS} />
//   </div>;
// }
// export default TestingArena;
...
import Button from "../components/Button";
import Checkbox, { CheckboxDynamic } from "../components/Checkbox";
import GlobalModal from "../components/GlobalModal";
import Table from "../components/Table";
import TemplateEdit from "../components/templates/TemplateEdit";
import { createOrder, getFilteredCustomers, getFilteredEventTypes, getFilteredQuotes } from "../config/httpRequests";
...
const CustomerModal = ({ show, onSubmit = (id, customer) => { }, onClose = () => { } }) => {
  const states = createStatesNew({
    searchTerm: "",
    searchRows: [],
    searchResult: [],
  ...
  function parseCustomersForTable(customer) {
    ...
    function onSelectCustomer(_, newValue) {
      states.get("searchSelected").set(newValue ? id : null);
      states.get("searchSelectedData").set(newValue ? structuredClone(customer) : null);
    }
  ...
  async function customersQuery(searchTerm) {
    ...
    if (!!(customers?.length ?? 0)) {
      const rows = customers.map(parseCustomersForTable);
